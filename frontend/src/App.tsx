...
type ProfileModel = { id: string; ownedBy?: string; enabled: boolean; isDefault: boolean; reasoningLevels: ReasoningLevel[]; reasoningCapabilitySource?: string; reasoningVerifiedLevels?: ReasoningLevel[]; reasoningRejectedLevels?: ReasoningLevel[]; reasoningControlUnsupported?: boolean; reasoningLastRequestedLevel?: ReasoningLevel; reasoningLastResolvedLevel?: ReasoningLevel; reasoningWireMode?: string };
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <section className="chat" aria-live="polite" ref={chatRef}>
        {messages.length === 0 ? <EmptyState hasProject={Boolean(projectId)} hasConversation={Boolean(conversationId)} hasProfile={Boolean(profileId && modelId)} openSettings={() => setSettingsOpen(true)} createConversation={() => setCreateDialog({ kind: "conversation", title: "", description: "", workspacePath: "" })} setPrompt={setInput}/> : <div className="message-stack">{messages.map((message) => <article key={message.id} className={`message ${message.role}`}><div className="avatar">{message.role === "user" ? "你" : <Icon name="spark" size={17}/>}</div><div className="message-body"><div className="message-meta"><b>{message.role === "user" ? "你" : selectedProfile?.name ?? "SciAide"}</b>{message.status === "incomplete" && <span>生成已中断</span>}</div><div className="bubble">{textOf(message) || (message.status === "streaming" && activeRun?.status !== "waiting_approval" ? <span className="typing"><i/><i/><i/> 正在生成回答</span> : "")}</div>{message.role === "assistant" && activeRun && message.runId === activeRun.id && <RunActivity run={activeRun} toolCalls={toolCalls} approvals={pendingApprovals} resolvingApprovalId={resolvingApprovalId} resolveApproval={resolveApproval}/>}</div></article>)}</div>}
      </section>
...
function ModelSettings({ profiles, close, refresh, select }: { profiles: Profile[]; close: () => void; refresh: () => Promise<void>; select: (id: string) => void }) {
  ...
  const [name, setName] = useState(""); const [apiProtocol, setAPIProtocol] = useState<APIProtocol>("openai_chat_completions"); const [baseUrl, setBaseUrl] = useState("https://api.openai.com/v1"); const [profileModels, setProfileModels] = useState<ProfileModel[]>([]); const [manualModelId, setManualModelId] = useState(""); const [apiKey, setApiKey] = useState(""); const [headers, setHeaders] = useState("");
  ...
  useEffect(() => { setName(current?.name ?? ""); setAPIProtocol(current?.apiProtocol ?? "openai_chat_completions"); setBaseUrl(current?.baseUrl ?? "https://api.openai.com/v1"); setProfileModels(current?.models ?? []); setManualModelId(""); setApiKey(""); setHeaders(current && Object.keys(current.customHeaders).length ? JSON.stringify(current.customHeaders) : ""); setModels([]); setModelSearch(""); setFeedback(null); }, [current]);
  ...
  function toggleModel(item: AvailableModel) {
    setProfileModels((currentModels) => {
      ...
      if (exists) {
        ...
        return remaining.map((model, index) => ({ ...model, isDefault: index === 0 ? !remaining.some((candidate) => candidate.isDefault) : model.isDefault }));
      }
      const declared = item.reasoningCapabilitySource === "provider" ? item.reasoningLevels ?? [] : [];
      const levels = declared.length ? declared : inferredReasoningLevels(apiProtocol, item.id);
      return [...currentModels, {
        ...
        reasoningCapabilitySource: declared.length ? "provider" : levels.length ? "inferred" : "unsupported",
      }];
    });
  }
  function addManualModel() {
    ...
    setProfileModels((currentModels) => [...currentModels, {
      id: value,
      enabled: true,
      isDefault: currentModels.length === 0,
  ...
  async function discover() { setDiscovering(true); setFeedback({ kind: "info", text: "正在读取 /v1/models…" }); try { const values = await backend<AvailableModel[]>("ModelFacade", "DiscoverModels", { profileId: id, apiProtocol, baseUrl, apiKey, customHeaders: parsedHeaders() }); setModels(values); setFeedback(values.length ? { kind: "ok", text: `已获取 ${values.length} 个模型，可勾选多个模型共用该 API Key。` } : { kind: "info", text: "服务返回了空列表，请手动添加 Model ID。" }); } catch (error) { setModels([]); setFeedback({ kind: "error", text: apiProtocol === "anthropic_messages" ? "Anthropic 原生服务通常不提供 /v1/models，请直接手动添加 Model ID。" : errorText(error) }); } finally { setDiscovering(false); } }
  async function save(event: FormEvent) { event.preventDefault(); const defaultModel = profileModels.find((model) => model.isDefault) ?? profileModels[0]; if (!defaultModel) { setFeedback({ kind: "error", text: "请至少选择或手动添加一个模型。" }); return; } setSaving(true); setFeedback(null); try { const saved = await backend<Profile>("ModelFacade", "SaveModelProfile", { id, name, apiProtocol, baseUrl, modelId: defaultModel.id, models: profileModels, apiKey, timeoutSeconds: current?.timeoutSeconds ?? 60, customHeaders: parsedHeaders(), enabled: true, isDefault: profiles.length === 0 || current?.isDefault === true }); await refresh(); setId(saved.id); select(saved.id); setApiKey(""); setFeedback({ kind: "ok", text: `配置和 ${profileModels.length} 个模型已安全保存。` }); } catch (error) { setFeedback({ kind: "error", text: errorText(error) }); } finally { setSaving(false); } }
